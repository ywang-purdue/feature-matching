...
imageName1 = 'figures/set3/pic1.jpg';
imageName2 = 'figures/set3/pic2.jpg';
...
for i = 1 : d1
    ...
    for j = 1 : d2
        ...
        img1block = img1block - mean(img1block(:));
        img2block = img2block - mean(img2block(:));
        num = sum(sum(img1block.*img2block));
        den = sqrt(sum(sum(img1block.^2))*sum(sum(img2block.^2)));
        e = abs(num/den - 1);
        if e < top1
            top2 = top1;
            top1 = e;
    ...
    if top1/top2 < 0.8 && top1 < 0.8
        plot([c1c(i),c2c(j_top)+600],[c1r(i),c2r(j_top)],'b-');
    end
